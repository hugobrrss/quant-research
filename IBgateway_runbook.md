...
_Reference for the headless, automated IB Gateway setup._

_Note: infrastructure identifiers (droplet IP, IB account and login ids) are redacted as `<...>` placeholders in this public copy._
...
Verified working after a **cold reboot** with zero manual steps: services came up, the gateway auto-logged-in, and an IB API client connected over localhost returning account `<paper-account>`. _(Originally verified with `ib_insync`; the stack migrated to the drop-in `ib_async` on 2026-06-15.)_
...
| Item | Value |
|------|-------|
| Droplet | DigitalOcean, IP `<droplet-ip>`, user `hugo`, Ubuntu 24.04.4 LTS |
...
| API port | `4002` (paper), account `<paper-account>`, login id `<paper-login>` |
...
- **ExistingSessionDetectedAction=primaryoverride.** The droplet reclaims the paper
  ...
  (`<paper-login>`) and live/paper data sharing is off (Share=No), it is decoupled from
  the live/Mac TWS identity — a manual live login no longer bumps the droplet (Phase 5, 2026-06-17).
...
The `stable-standalone` build **never self-updates** — `1045` stays `1045` until deliberately changed. This is to avoid the gateway silently changing version mid-week and breaking IBC or code.

To update (or IBKR enforces a minimum version):
